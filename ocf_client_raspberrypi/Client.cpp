...
#define SENSOR_RESOURCE_URI		"/WaterValueURI"

#define LIGHT_RESOURCE_KEY		"value"
#define SENSOR_RESOURCE_KEY		"value"
		...
		const int& sequenceNumber)
{
    ...
    try
    {
        ...
        if(eCode == OC_STACK_OK && sequenceNumber <= MAX_SEQUENCE_NUMBER)
        {
			...
			{
				_sensor_model.SetValue(rep, SENSOR_RESOURCE_KEY);
				cout << "\twater: " << _sensor_model.GetValue() << endl;
			}
			...
			if(_registered_resources[LIGHT_RESOURCE_URI])
			{
				...
				if(_sensor_model.GetValue() == true && _light_model.GetValue() == false)
					value = true;

				if(_sensor_model.GetValue() == false && _light_model.GetValue() == true)
					value = false;
...
static void onGet(const HeaderOptions &headerOptions, const OCRepresentation &rep, const int eCode)
{
	...
	try
	{
		if(eCode == OC_STACK_OK)
		{
			...
			{
				_sensor_model.SetValue(rep, SENSOR_RESOURCE_KEY);
				cout << "\tclean: " << _sensor_model.GetValue() << endl;
